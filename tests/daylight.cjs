...
test('night runs from sunset to next sunrise without resetting at either midnight', () => {
  ...
  h.deliver(data);
  const markerX = () => h.calls.find(c => c.kind === 'rect' && c.color === 0xffffff && c.width === 11 && c.y === 195)?.x;
  h.tick('2026-09-10T00:10:00Z');
  assert.deepEqual(h.texts().filter(t => /^(RISE|SET) /.test(t)), ['SET 19:00', 'RISE 08:00']);
  const afterSunset = markerX();
  assert.ok(afterSunset !== undefined, 'night marker is drawn after sunset');
  assert.ok(!h.calls.some(c => c.kind === 'rect' && c.y === 200 && ![0, 0xffffff].includes(c.color)), 'night ruler is black like the day ruler');
  h.tick('2026-09-10T04:59:00Z');
  const beforeMidnight = markerX();
  assert.ok(beforeMidnight > afterSunset, 'night marker advances');
  h.tick('2026-09-10T05:01:00Z');
  assert.ok(markerX() >= beforeMidnight, 'progress continues across location midnight');
  assert.deepEqual(h.texts().filter(t => /^(RISE|SET) /.test(t)), ['SET 19:00', 'RISE 08:00']);
...
test('adjacent HUD digits get a pixel of air so 24 does not read as one glyph', () => {
  const h = boot();
  assert.equal(h.eval('trackedWidth("THU SEP 24", dateFont) - render.getTextWidth("THU SEP 24", dateFont)'), 1);
  assert.equal(h.eval('trackedWidth("RISE 06:55", smallFont) - render.getTextWidth("RISE 06:55", smallFont)'), 2);
  assert.equal(h.eval('trackedWidth("THU SEP 9", dateFont) - render.getTextWidth("THU SEP 9", dateFont)'), 0);
});
